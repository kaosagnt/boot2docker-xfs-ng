...
# updated via "update.sh"
ENV VBOX_VERSION 5.2.24
# https://www.virtualbox.org/download/hashes/$VBOX_VERSION/SHA256SUMS
ENV VBOX_SHA256 cb217dbc3a34862d36ee0a5f5a6b0756e7b639c4852e675eeec7c4422dc02599
...
# rngd for extra entropy on boot.
...
#last state.
#
# Everything requiring crypto and strong entropy during boot.
# Add the rng-tools and use the rngd to overcome this issue.
# Behaviour exhibited running on Virtualbox and VMWare Fusion.
ENV RNGD v6.6

ADD files/rngd usr/local/bin/

RUN set -ex; \
	mkdir -p /rng-tools; \
	wget -O - "https://codeload.github.com/nhorman/rng-tools/tar.gz/${RNGD}" \
		| tar -xzC /rng-tools/ --strip-components 1 ; \

	( cd /rng-tools/ && ./autogen.sh && ./configure ) ; \
	make -C /rng-tools/ ; \
	strip /rng-tools/rngd ; \
	make -C /rng-tools/ check ; \

	mkdir -p usr/lib/rng-tools; \
	cp -v "/rng-tools/rngd" "usr/local/bin/rngd-deb" ; \

	# /usr/lib/x86_64-linux-gnu/
	for usrlibtocopy in libcrypto.so.1.0.2 \
						libssl.so.1.0.2 \
						libnghttp2.so.14 \
						libidn2.so.0 \
						librtmp.so.1 \
						...
						libcurl.so.4.4.0 \
	; do \
		cp -v "/usr/lib/x86_64-linux-gnu/${usrlibtocopy}" \
			"usr/lib/rng-tools/"; \
	done; \
	# end /usr/lib/x86_64-linux-gnu/

	# /lib/x86_64-linux-gnu/
	for libtocopy in libgcrypt.so.20 \
						libgcrypt.so.20.1.6 \
						libkeyutils.so.1 \
						libkeyutils.so.1.5 \
						libidn.so.11 \
						libidn.so.11.6.16 \
						libgpg-error.so.0 \
						libgpg-error.so.0.21.0 \
	; do \
		cp -v "/lib/x86_64-linux-gnu/${libtocopy}" \
			"usr/lib/rng-tools/"; \
	done; \
	# end /lib/x86_64-linux-gnu/

	( cd usr/lib/rng-tools/ \
	&& ln -s libcurl.so.4.4.0 libcurl.so \
	&& ln -s libcurl.so.4.4.0 libcurl.so.4 ) ; \

	tcl-chroot /usr/local/bin/rngd --version
